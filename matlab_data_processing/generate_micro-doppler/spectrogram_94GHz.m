...
save_folder = fullfile('H:\extra_training_data\94GHz\cluster',['interval_' num2str(time_interval) '_wd_' num2str(window_length)]);
label_path = 'cluster94GHz.csv';
...
%     num_points_interval, ...
%     num_points_overlap, ...
%     floor(35/range_resolution), ...
%     ceil(40/range_resolution), ...
%     range_bins, ...
%     chirp_period, ...
%     lambda, ...
%     strength_from, ...
...
function gen_spectrogram( ...
    ...
    window_length, ...
    width_factor)
fh = fopen(fullfile(data_foldername,[str '_94GHz_FMCW.dat']),'r');
data = fread(fh, inf, 'uint16');
data = complex(data(1:2:end) - 32768,data(2:2:end) - 32768);
...
for start_pos=positions
    ...
    for n=1:tchirp-1
        ...
        diff=a3(3)-phase_unwrapped((n*cpi)+3);
    ...
    for m= 1:1
        for k=1:ns
            tmp=win' .* x_i_int(m,k:k+window_length-1);
            tmp=fft(tmp);
    ...
    colormap(jet(256))
    imagesc([0,77.269e-6*np],velocity, (sumdB));
